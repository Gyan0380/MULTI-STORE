...
function openBuyModal(catKey, itemIdx) {
    ...
    const opts = item.buyOptions || {online:true, discord:true, wa:true, tg:true, fb:false}; // Fallback
    ...
    // Set visibility based on Admin settings
    ...
    // Set Dynamic Links from Global Settings
...
// ⚡ ONLINE CHECKOUT WITH DYNAMIC QR
...
auth.getRedirectResult().then((r) => { if(r && r.user) switchView('welcome-view'); }).catch((e) => { if(e.code === 'auth/unauthorized-domain') alert("⚠️ ADD VERCEL DOMAIN TO FIREBASE!"); });
